...
class AnalyticsService {
    ...
    private function set_last_viewed_version_cookie($postid,$versionid){
        ...
        if(isset($postid) && isset($versionid)){
            ...
            \IfSo\PublicFace\Helpers\CookieConsent::get_instance()->set_cookie($this->last_viewed_version_cookie_name,json_encode($viewed_arr),0,'/');
        }
    }
        ...
        if($this->isOn){
            if(!isset(self::$viewed_triggers[$rule_data->get_trigger_id()]) && $this->allow_counting){
                ...
                if(!$this->useAjax){
                    ...
                    \IfSo\PublicFace\Helpers\CookieConsent::get_instance()->set_cookie($this->currently_viewing_cookie_name,json_encode(self::$viewed_triggers),0,'/');
                }
            }
        }
    }
