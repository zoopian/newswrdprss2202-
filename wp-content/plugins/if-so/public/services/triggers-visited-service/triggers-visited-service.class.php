...
class TriggersVisitedService {
    ...
    private function __construct() {
        ...
        if($cookie_content !== json_encode($this->visited_triggers) && $this->use_cookie)
            \IfSo\PublicFace\Helpers\CookieConsent::get_instance()->set_cookie($this->cookie_name,json_encode($this->visited_triggers),time()+60*60*24*365,'/');
    }
    ...
    public function set_visited_triggers(){
        if($this->use_cookie)
            \IfSo\PublicFace\Helpers\CookieConsent::get_instance()->set_cookie($this->cookie_name,json_encode($this->visited_triggers),0,'/');
    }
