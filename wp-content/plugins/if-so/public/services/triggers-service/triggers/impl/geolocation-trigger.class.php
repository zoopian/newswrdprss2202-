...
class GeolocationTrigger extends TriggerBase {
	...
	public function handle($trigger_data) {
		...
		$geolocation_data = (!empty($rule['is_standalone_condition']) && $rule['is_standalone_condition']) ? $rule['geolocation_data'] :  utf8_decode($rule['geolocation_data']);   //In the triggers , the geolocation data is utf8-encoded
